...
export function BuildList({ builds = [] }) {
  ...
  const [displayBuilds, setDisplayBuilds] = useState([])

  useEffect(() => {
    // If builds are provided as props, use them
    if (builds.length > 0) {
    ...
    if (savedBuilds) {
      setDisplayBuilds(JSON.parse(savedBuilds))
    } else {
      ...
      setDisplayBuilds(defaultBuilds)
      localStorage.setItem("builds", JSON.stringify(defaultBuilds))
    }

    // Set up event listener for storage changes
    const handleStorageChange = (e) => {
      if (e.key === "builds") {
        const updatedBuilds = JSON.parse(e.newValue)
        setDisplayBuilds(updatedBuilds)
      }
    }
